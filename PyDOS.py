...
def config():
    ...
    if CONFIG[3] == "true" or "true\n": show_stats = True
    else: show_stats = False
    if CONFIG[4] == "true" or "true\n": ask_for_drives = True
...
while True:
    ...
    # help
    if action[0] == "help":
        if command_check(action, 1) == False:
            print("PyDOS Help:")
    ...
    # terminates PY-DOS
        ...
        if command_check(action, len(action)) == False:
            if len(action) == 2:
                if action[1] == "-y":
                    quit()
            else:
    ...
    # opens files
        ...
        if command_check(action, 3) == False:
            # prints files
